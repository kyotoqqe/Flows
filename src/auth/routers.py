...
def get_spotify_oauth_redirect_uri():
    uri = generate_spotify_oauth_redirect_uri()
    return uri

#for test
@router.post("/spotify/callback")
async def get_spotify_code(code: Annotated[str, Body(embed=True)]):
    spotify_token_url = "https://accounts.spotify.com/api/token"

    #create one on app
    async with aiohttp.ClientSession() as session:
        form_data = aiohttp.FormData()
        form_data.add_field(name="grant_type", value="authorization_code")
        form_data.add_field(name="code", value=code)
        form_data.add_field(name="redirect_uri", value="http://127.0.0.1:8000/api/auth/spotify/callback")

        base64_auth_str = base64.b64encode(b"01da06ec10a84823b6cbd8d7e8e8a4ae:4d4530212a784580ad6f6da7772d5dc1")

        headers = {
            "content-type": "application/x-www-form-urlencoded",
            "Authorization": f"Basic {base64_auth_str.decode()}"
        }
